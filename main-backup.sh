...
$(which rsync) $rsync_sync_opt $(katahdin)Uploads/2023/ $(gamehenge)Photography/ALL\ UPLOADS/2023/ >> $(log_path)backup_$current_date.log
$(which rsync) $rsync_sync_opt $(katahdin)Lightroom/ $(gamehenge)Photography/Lightroom/NEW_Lightroom_Backups/ >> $(log_path)backup_$current_date.log
$(which rsync) $rsync_sync_opt $(katahdin)PhotosLibrary $(gamehenge)Photography/ >> $(log_path)backup_$current_date.log

echo "syncing volume gamehenge to catalyst"
$(which rsync) $rsync_sync_opt --exclude=Movies_Shows $(gamehenge) $(catalyst) >> $(log_path)backup_$current_date.log

echo "Backing up gamehenge to abraxas"
rsync-backup.sh $rsync_bkup_opt --exclude=TOM $(gamehenge) $(abraxas) >> $(log_path)backup_$current_date.log
